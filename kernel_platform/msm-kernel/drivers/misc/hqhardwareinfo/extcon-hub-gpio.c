...
static int qpnp_usb_hub_probe(struct platform_device *pdev)
{
	...
	if (ret) {
        ...
        //set hub/cdb/skyworks as poweroff state when booting at charger mode
        if (!is_charger_mode()) {
            queue_delayed_work(system_power_efficient_wq, &chip->hub_detect_work,
                msecs_to_jiffies(USB_HUB_START_DETECT_MS));
        }
