...
static void tp_enter_charge_work(struct work_struct *work)
{
    int ret = 0;
    ...
    if (fts_data->charger_mode) {
        ...
        if (ret < 0) {
            FTS_ERROR("MODE_CHARGER enter  fail");
        }
    } else {
        ret = fts_write_reg(FTS_REG_CHARGER_MODE_EN, 0);
        if (ret < 0) {
            FTS_ERROR("MODE_CHARGER exit  fail");
        }
    }
    return;
}

void tp_switch_chage_mod(bool mode)
{
    struct fts_ts_data *ts_data = fts_data;

    if ((charge_active == 0) || (fts_data == NULL)) {
        return;
    }

    fts_data->charger_mode = mode;

    queue_work(fts_data->ts_workqueue,&ts_data->charge_mode_work);

}

EXPORT_SYMBOL(tp_switch_chage_mod);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    if (pdata->reset_gpio < 0) {
        if (pdata->reset_gpio != -EPROBE_DEFER) {
            FTS_ERROR("Unable to get reset-gpio %d", pdata->reset_gpio);
        } else {
            FTS_ERROR("reset-gpio not ready %d", pdata->reset_gpio);
        }
    ...
    if (pdata->irq_gpio < 0) {
        if (pdata->irq_gpio != -EPROBE_DEFER) {
            FTS_ERROR("Unable to get irq-gpio %d", pdata->irq_gpio);
        } else {
            FTS_ERROR("irq-gpio not ready %d", pdata->irq_gpio);
        }
				...
				void *pvt_data)
{
    ...
    struct fts_ts_data *ts_data = pvt_data;
    ...
    pr_err("Notification type:%d, early_trigger:%d",
  	...
  	case DRM_PANEL_EVENT_BLANK_LP:
  		pr_err("received lp event\n");
  		break;
  
  	case DRM_PANEL_EVENT_FPS_CHANGE:
  		pr_err("Received fps change old fps:%d new fps:%d\n",
  			notification->notif_data.old_fps,
  			notification->notif_data.new_fps);
  		break;
  	default:
  		pr_err("notification serviced :%d\n",
  			notification->notif_type);
  		break;
  	}
...
//    get_hardware_info_data(HWID_CTP_DRIVER, FTS_DRIVER_VERSION);//get hw_info of tp
    ...
    if (ret) {
     ...
     if (fts_data->charger_mode) {
        fts_write_reg(FTS_REG_CHARGER_MODE_EN, 1);
    }
    ...
    if (ts_data->ts_workqueue) {
        ...
        INIT_WORK(&ts_data->charge_mode_work, tp_enter_charge_work);
    }
...
static bool is_charger_mode(void)
{
	...
	if (strstr(bootargs,"androidboot.mode=charger")){
		pr_info("%s: boot mode:charger",__func__);
		ret = true;
	} else {
		pr_err("%s: fail to get androidboot.mode=charger in bootargs!",__func__);
		ret = false;
	}
	return ret;
}
...
{
    int ret = 0;
    printk("[TP] %s the data  INIT enter\n", __func__);
    //if it is in charger mode,then do not register the TP.
    if (is_charger_mode()) {
	FTS_ERROR("Focaltech touch mode is charge mode status, return...\n");
	return ret;
    }
    FTS_FUNC_ENTER();
    ...
    if ( ret != 0 ) {
        ...
        printk("[TP] %s Focaltech touch screen driver init failed\n", __func__);
    }
    FTS_FUNC_EXIT();
    printk("[TP] %s the data  INIT exit\n", __func__);
    return ret;
}
