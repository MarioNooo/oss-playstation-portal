...
/* fts_test_result interface */
static ssize_t fts_test_result_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
//	int ret = 0;
    	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
	...
	if (g_fts_test_result == true)
	{
		num_read_chars = snprintf(buf, PAGE_SIZE, "fts test pass!\n");
	}
	else
	{
...
static ssize_t fts_test_result_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
...
static DEVICE_ATTR(fts_test_result, S_IRUGO | S_IWUSR, fts_test_result_show, fts_test_result_store);
